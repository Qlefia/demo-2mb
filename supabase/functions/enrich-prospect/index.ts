/**
 * Proxies enrichment to the deployed CRM app using a shared secret.
 *
 * Supabase secrets (Dashboard → Edge Functions → Secrets):
 * - ENRICH_INTERNAL_SECRET — same value as ENRICH_INTERNAL_SECRET on Vercel / backend
 * - CRM_API_BASE_URL — origin only, e.g. https://your-app.vercel.app (no trailing slash)
 *
...
Deno.serve(async (req: Request) => {
  ...
  const res = await fetch(`${base}/api/prospects/${id}/enrich`, {
    method: 'POST',
    headers: {
